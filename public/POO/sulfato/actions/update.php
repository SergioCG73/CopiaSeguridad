...
if ($TipoUpdate == "1") {  //Se actualiza Pesos o Notas
    ...
    $sql = $conexion->prepare("UPDATE sulfato SET Peso_Inicial = $PesoInicial, Peso_Final = $PesoFinal, Notas = '$Notas' 
...
if ($TipoUpdate == "2"){   //Se actualiza el REACTOR
    ...
    $sql = $conexion->prepare("SELECT NumeroFabricacion, Hora_Finalizacion FROM sulfato 
    ...
    $sql = $conexion->prepare("UPDATE sulfato SET Reactor = '$Reactor', Tiempo_Parado = $TiempoParado 
                               WHERE NumeroFabricacion = '$NumeroFabricacion'");
    $updateProduccion = $sql->execute();
...
if ($TipoUpdate == "3"){ //Se actualiza FECHA INICIAL
    ...
    $sql = $conexion->prepare("UPDATE sulfato SET Hora_Inicio = '$FechaInicialOutput', Duracion = $Duracion, Tiempo_Parado = $TiempoParado 
                               WHERE NumeroFabricacion = $NumeroFabricacion") ;
    $updateProduccion = $sql->execute();
    echo json_encode($update); 
}
...
$sql = $conexion->prepare("UPDATE sulfato SET Hora_Finalizacion = '$FechaFinalOutput', Duracion = $Duracion 
                           WHERE NumeroFabricacion = $NumeroFabricacion");
$sql->execute();
...
$sql = $conexion->prepare("SELECT NumeroFabricacion, Hora_Inicio FROM sulfato
                           WHERE NumeroFabricacion > $NumeroFabricacion AND Reactor = '$Reactor'
                           LIMIT 1");
...
// --------------------------- FIN CÁLCULO TIEMPO PARADO ----------------------------------    
    ...
    $sql = $conexion->prepare("UPDATE sulfato SET Tiempo_Parado = $TiempoParado WHERE NumeroFabricacion = $NumeroFabricacionSQL");
    $updateProduccion = $sql->execute();   
    echo json_encode($update);
}
}

document.addEventListener("DOMContentLoaded", () => {
    ...
    let storedValue = localStorage.getItem("selectedRegistrosFerrico");
    ...
    select.addEventListener("change", () => {
        registros = select.options[select.selectedIndex].value;
        localStorage.setItem("selectedRegistrosFerrico", registros); // Guardar el valor seleccionado en localStorage
    ...
    function FormatearDatos(response) {
        response.forEach(elemento => {
          ...
          // Formatear ÁCIDO LIBRE
          let AcidoLibreFormateado = Number(elemento.Acido);
          AcidoLibreFormateado = AcidoLibreFormateado.toLocaleString('es-ES', {minimumFractionDigits: 2, maximumFractionDigits: 2});  
            ...
            tbody.innerHTML += `
                                <tr>
                                    ...
                                    <td> <a href="actions/delete.php?id=${elemento.NumeroFabricacion}" onclick="return confirm('¿Estás seguro de que deseas borrar este registro?');">
                                         ...
                                         Volumen_Inicial=${elemento.Volumen_Inicial}&
                                         Volumen_Final=${elemento.Volumen_Final}&

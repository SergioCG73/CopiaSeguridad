<?php
   function miErrorHandler($errno, $errstr, $errfile, $errline) { // Definir una función para manejar errores
    ...
    if ($search_criteria !="") {
        $registros = "100";
        $sql = $conexion->prepare("SELECT * FROM sulfacid
                                    WHERE NumeroFabricacion LIKE '%" . $search_criteria . "%'                                   
                                    OR Fecha LIKE '%" . $search_criteria . "%'
                                    ORDER BY NumeroFabricacion DESC
                                    LIMIT " . $registros);
    }
    else {
        $sql = $conexion->prepare("SELECT * FROM sulfacid ORDER BY NumeroFabricacion DESC LIMIT $registros");
    }
